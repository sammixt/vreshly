...
$(function () {
  ...
  //    emailUserList.find('li').on('click', '.message_item', function (e) {
  ...
  //}
    ...
    $('.email-media-list').on('click', '.message_item', function (e) {
        let _id = $(this).attr("data-id");
        axios.get(`${url}AdminMail/GetMessage?msgId=${_id}`)
            .then(response => {
                const res = response.data;
                $('.email-subject').text(res.subject);
                $('.fullname').text(res.fullName);
                $('.email').text(res.email);
                $('.datetime').text(res.dateCreated);
                $('.timedate').text(res.timeCreated);
                $('.message_body').text(res.body);

                $('.trash-item').attr('data-trash', _id);
    ...
    $('.trash-item').on('click', function (e) {
        e.preventDefault();
        let _id = $(this).attr("data-trash");
        if (deleteMail(_id))
            window.location.reload();
    })

    var deleteMail = function (_id) {
        let callback = false;
        axios.get(`${url}AdminMail/Delete?msgId=${_id}`)
            .then(response => {
                callback = true;
            })
            .catch(function (error) {
  ...
  if (mailDelete.length) {
    mailDelete.on('click', function () {
      if (userActions.find('.custom-checkbox input:checked').length) {
          var media = userActions.find('.custom-checkbox input:checked').closest('.media');//.remove();
          var _id = media.attr('data-id');
          if (deleteMail(_id)) {
              media.remove();
...
$(window).on('load', function () {
    axios.get(`${url}AdminMail/CountMessage`)
        .then(response => {
            const res = response.data;
            $('.inbox-count').text(res.inboxCount);
            $('.trash-count').text(res.trashCount);
            $('.sent-count').text(res.sentCount);
        })
        .catch(function (error) {
            if (error.response) {
                toastr['error'](error.response.data.message, 'Error!', {
