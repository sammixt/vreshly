$(function () {
    ...
    var addToCart = function (_id, _productName, _price, _quantity, _prictureUrl, _brand, _category) {
        ...
        toast('success', `${_productName} added to cart`);
        cartCount();
        loadCartDropdown();
    ...
    addProductToCart.on('click', '.add-to-cart', function (e) {
        ...
        if (parseInt(qty) > 0) {
            ...
            toast('error', `Quantity must be greater than Zero (0)`);
        }
    });
    ...
    wishlistProduct.on('click', '.add-to-cart', function (e) {
        ...
        if (_quantity > 0) {
            ...
            toast('error', `${_productName} is out of stock`);
        }
    });

    $('#exampleModalCenter .modal-body').on('click', '.add-to-cart-modal', function () {
        var $button = $(this);
        var qty = $button.parent().parent().find('input').val();
        if (parseInt(qty) > 0) {
            var _id = $button.attr('data-id');
